...
def main(unused_args, flags):
  ...
  for src in all_srcs_list:
    ...
    clean_src = src #sanitize_xml(src)
    replace_srcs += '            <File>\n'
    replace_srcs += '              <FileName>' + basename + '</FileName>\n'
    replace_srcs += '              <FileType>' + ext_index + '</FileType>\n'
    replace_srcs += '              <FilePath>' + clean_src + '</FilePath>\n'
    replace_srcs += '            </File>\n'
  template_file_text = re.sub(r'%{SRCS}%', 'Source',
                              six.ensure_str(template_file_text))

  template_file_text = re.sub(r'%{SOURCES}%', replace_srcs,
                              six.ensure_str(template_file_text))	

  defines = re.sub(' ', ',', six.ensure_str(flags.defines.lstrip()))
  template_file_text = re.sub(r'%{DEFINE}%', defines,
                              six.ensure_str(template_file_text))						  
